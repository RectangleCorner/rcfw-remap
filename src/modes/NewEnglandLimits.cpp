...
uint8_t isEasy3(const uint8_t x, const uint8_t y) {
    // is it on the rim?
    const uint8_t xnorm =
...
uint8_t getRandom3(uint16_t currentTime) {
...
// 1 2 1 //4
// totals up to 16
void randomizeCoord3(uint8_t &x, uint8_t &y, uint16_t currentTime) {
    const uint8_t random = getRandom3(currentTime);
...
uint8_t lookback3(const uint8_t currentIndex, const uint8_t samplesBack) {
...
uint8_t sdiZone3(const uint8_t x, const uint8_t y) {
...
uint8_t pivotZone3(const uint8_t x) {
    uint8_t result = 0b0000'0000;
...
uint8_t uptiltShutoffZone3(const uint8_t y) {
    uint8_t result = 0b0000'0000;
...
uint8_t popcount_zone3(const uint8_t bitsIn) {
    uint8_t count = 0;
...
uint8_t isTapSDI3(
    ...
    for (int i = 0; i < historyLength; i++) {
        const uint8_t index = lookback3(currentIndex, i);
        zoneList[i] = zoneHistory[index].zone;
        timeList[i] = zoneHistory[index].timestamp;
        staleList[i] = zoneHistory[index].stale;
    }
    const uint8_t popCur = popcount_zone3(zoneList[0]);
    const uint8_t popOne = popcount_zone3(zoneList[1]);
    ...
    uint8_t diagCount = 0;
    for (int i = 0; i < historyLength; i++) {
        const uint8_t popcnt = popcount_zone3(zoneList[i]);
        if (popcnt == 0) {
            origCount++;
        } else if (popcnt == 1) {
            cardCount++;
    ...
    { // to limit scope of these vars
        ...
        const bool diagMatch = popcount_zone3(diagZone) == 2;
    ...
    { // to limit scope of these vars
        // check the bit count of diagonal matching
        const bool adjacentDiag = popcount_zone3(diagZone & cardZone) == 1;
    ...
    if (!output) {
        ...
        if ((zoneList[0] == zoneList[2]) && (popcount_zone3(zoneList[0]) == 2)) {
            // check duration
    ...
    for (int i = 0; i < historyLength; i++) {
        if ((popcount_zone3(zoneList[i]) == 2) && !alreadyWritten) {
            lookNow = true;
        }
        if ((popcount_zone3(zoneList[i]) == 1) && !alreadyWritten && lookNow) {
...
void limitOutputsNewEngland(
    ...
    if (inputs.lf4 /*l*/ || inputs.rf5 /*r*/) {
        ...
        if (aHistory[currentIndexA].x != rawOutputIn.leftStickX ||
            ...
            if (((yInMag * 157 < xInMag * 80) || (xInMag * 157 < yInMag * 80)) && xInMag &&
                ...
                // fuzz it
                randomizeCoord3(x_end, y_end, currentTime);
                // write it back
    ...
    const uint8_t tapSDI = isTapSDI3(sdiZoneHist, currentIndexSDI, currentTime, sampleSpacing);
    // if cardinal tap SDI
    ...
    if (pivotZoneHist[0].zone != pivotZone3(prelimAX)) {
        ...
        pivotZoneHist[0].zone = pivotZone3(prelimAX);
    ...
    */
    ...
    if (sdiZoneHist[currentIndexSDI].zone !=
        sdiZone3(rawOutputIn.leftStickX, rawOutputIn.leftStickY)) {
    ...
    if (prevX != xIn || prevY != yIn) {
        ...
        if (!wavedashSkip) {
            ...
            uint8_t xInRand = xIn;
            uint8_t yInRand = yIn;
            randomizeCoord3(xInRand, yInRand, currentTime);
            ...
            const uint8_t easiness = isEasy3(xIn, yIn);
            if (easiness == 1) {
