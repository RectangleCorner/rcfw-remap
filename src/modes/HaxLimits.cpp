...
// crouch uptilt disabled
#define TIMELIMIT_DOWNUP \
    (0) // units of 4us; how long after a crouch to upward input should it begin a jump?
#define JUMP_TIME \
    (0) // units of 4us; after a recent crouch to upward input, always hold full up for 2 frames

// crouch uptilt enabled
// #define TIMELIMIT_DOWNUP \
//     (16 * 3 * 250) // units of 4us; how long after a crouch to upward input should it begin a
//     jump?
// #define JUMP_TIME \
//     (16 * 2 * 250 \
...
#define TRAVELTIME_SLOW (0) // ms for tap SDI nerfing, 4 frames

#define TIMELIMIT_DEBOUNCE (0) // units of 4us; 6ms;
#define TIMELIMIT_SIMUL \
    (0) // units of 4us; 3ms: if the latest inputs are less than 2 ms apart then don't nerf cardiag
...
#define TIMELIMIT_TAP 0 //(16*5.5*250)//units of 4us; 5.5 frames
#define TIMELIMIT_TAP_PLUS 0 //(16*8.5*250)//units of 4us; 3 additional frames

#define TIMELIMIT_CARDIAG 0 //(16*8*250)//units of 4us; 8 frames

// everything below is travel time related
...
// #define TIMELIMIT_DEBOUNCE (0) // units of 4us; 6ms;
// #define TIMELIMIT_SIMUL (0) // units of 4us; 3ms: if the latest inputs are less than 2 ms apart
// then don't nerf cardiag
...
uint8_t isEasy2(const uint8_t x, const uint8_t y) {
    // is it on the rim?
    const uint8_t xnorm =
...
uint8_t getRandom2(uint16_t currentTime) {
...
// 1 2 1 //4
// totals up to 16
void randomizeCoord2(uint8_t &x, uint8_t &y, uint16_t currentTime) {
    /*
...
uint8_t lookback2(const uint8_t currentIndex, const uint8_t samplesBack) {
...
uint8_t sdiZone2(const uint8_t x, const uint8_t y) {
...
uint8_t pivotZone2(const uint8_t x) {
    uint8_t result = 0b0000'0000;
...
uint8_t uptiltShutoffZone2(const uint8_t y) {
    uint8_t result = 0b0000'0000;
...
uint8_t popcount_zone2(const uint8_t bitsIn) {
    uint8_t count = 0;
...
uint8_t isTapSDI2(
    const sdizonestate2 zoneHistory[HISTORYLEN],
    ...
    for (int i = 0; i < historyLength; i++) {
        const uint8_t index = lookback2(currentIndex, i);
        zoneList[i] = zoneHistory[index].zone;
        timeList[i] = zoneHistory[index].timestamp;
        staleList[i] = zoneHistory[index].stale;
    }
    const uint8_t popCur = popcount_zone2(zoneList[0]);
    const uint8_t popOne = popcount_zone2(zoneList[1]);
    ...
    if (zoneList[0] != zoneList[1] && (zoneList[0] == zoneList[2]) &&
        ...
        if (!staleList[3] && (timeDiff0 < TIMELIMIT_TAP_PLUS && timeDiff1 < TIMELIMIT_TAP &&
                              timeDiff0 > TIMELIMIT_DEBOUNCE)) {
            if ((zoneList[0] == 0) ||
                (zoneList[1] == 0
                )) { // if one of the pairs of zones is zero, it's tapping a cardinal
    ...
    uint8_t diagCount = 0;
    for (int i = 0; i < historyLength; i++) {
        const uint8_t popcnt = popcount_zone2(zoneList[i]);
        if (popcnt == 0) {
            origCount++;
        } else if (popcnt == 1) {
            cardCount++;
    ...
    { // to limit scope of these vars
        ...
        const bool diagMatch = popcount_zone2(diagZone) == 2;
    ...
    { // to limit scope of these vars
        // check the bit count of diagonal matching
        const bool adjacentDiag = popcount_zone2(diagZone & cardZone) == 1;
    ...
    for (int i = 0; i < historyLength; i++) {
        if ((popcount_zone2(zoneList[i]) == 2) && !alreadyWritten) {
            lookNow = true;
        }
        if ((popcount_zone2(zoneList[i]) == 1) && !alreadyWritten && lookNow) {
...
void travelTimeCalc2(
    const uint16_t samplesElapsed,
...
void limitOutputsHax(
    ...
    static shortstate2 aHistory[HISTORYLEN];
    static sdizonestate2 sdiZoneHist[HISTORYLEN];
    static pivotzonestate2 pivotZoneHist[HISTORYLEN];
    ...
    if (!initialized) {
        ...
        prevInputs.lf4 = inputs.lf4; // we don't care about recording these
        prevInputs.lf3 = inputs.lf3;
        prevInputs.lf1 = inputs.lf1;
        prevInputs.lf2 = inputs.lf2;

        prevInputs.rf4 = inputs.rf4;
        prevInputs.rt2 = inputs.rt2;
        prevInputs.rt3 = inputs.rt3;
        prevInputs.rt4 = inputs.rt4;
        prevInputs.rt5 = inputs.rt5;
        prevInputs.rf1 = inputs.rf1;
        prevInputs.rf6 = inputs.rf6;
        prevInputs.rf7 = inputs.rf7;
        prevInputs.rf8 = inputs.rf8;
        prevInputs.lt1 = inputs.lt1;
        prevInputs.lt2 = inputs.lt2;
    ...
    if (inputs.lf4 || inputs.rf5) {
        ...
        if (aHistory[currentIndexA].x != rawOutputIn.leftStickX ||
            ...
            if (((yInMag * 157 < xInMag * 80) || (xInMag * 157 < yInMag * 80)) && xInMag &&
                ...
                // fuzz it
                randomizeCoord2(x_end, y_end, currentTime);
                // write it back
                ...
                if (!(prevInputs.lf4 || prevInputs.rf5)) {
...
prevInputs.lt2 == inputs.lt2
    ...
    } else if (!(inputs.lf4 || inputs.rf5) && (prevInputs.lf4 || prevInputs.rf5)) {
    ...
    travelTimeCalc2(
        currentTime - aHistory[currentIndexA].timestamp,
        sampleSpacing,
    ...
    if (pivotZoneHist[0].zone != pivotZone2(prelimAX)) {
        ...
        pivotZoneHist[0].zone = pivotZone2(prelimAX);
    ...
    */
    ...
    const uint8_t sdi = isTapSDI2(sdiZoneHist, currentIndexSDI, currentTime, sampleSpacing);
    ...
    if (aHistory[currentIndexA].x != rawOutputIn.leftStickX ||
        ...
        if (!wavedashSkip) {
            ...
            uint8_t xInRand = xIn;
            uint8_t yInRand = yIn;
            randomizeCoord2(xInRand, yInRand, currentTime);
            ...
            const uint8_t easiness = isEasy2(xIn, yIn);
            if (easiness == 1) {
