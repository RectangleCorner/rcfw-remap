#include "core/modz_mode_selection.hpp"
    ...
    switch (mode_config.mode_id) {
        case MODE_MELEE:
            if (config.travelTime == true) {
                ...
                melee_mode.SetConfig(mode_config, config.melee_options, config.meleeCWOS);
        ...
        case MODE_WINGMANFGC:
            ...
            break;
        case MODE_MELEE_MODZ:
            melee_modZ.SetConfig(mode_config, config.melee_options);
            set_mode(backend, &melee_modZ);
            break;
        // case MODE_MULTIVERSUS:
        //     mvs.SetConfig(mode_config);
        //     set_mode(backend, &mvs);
        //     break;
        case MODE_KEYBOARD:
            if (backend->BackendId() != COMMS_BACKEND_DINPUT ||
